export default async function handler(req, res) {
  ...
  try {
    ...
    if (data.error) {
      var keyPreview = GEMINI_KEY ? (GEMINI_KEY.substring(0,7) + '...' + GEMINI_KEY.substring(GEMINI_KEY.length-4) + ' (largo: ' + GEMINI_KEY.length + ')') : 'NO CONFIGURADA';
      data.error.message = (data.error.message || JSON.stringify(data.error)) + ' | DIAGNOSTICO clave usada: ' + keyPreview;
